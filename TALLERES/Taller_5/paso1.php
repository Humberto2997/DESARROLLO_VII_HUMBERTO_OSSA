...
function contarCiudadesPorInicial($arr, $inicial) {
    $contador = 0;
    // Convertimos la inicial a mayúscula para comparar sin importar minúsculas/mayúsculas
    $inicial = strtoupper($inicial);

    foreach ($arr as $ciudad) {
        if (strtoupper(substr($ciudad, 0, 1)) === $inicial) {
            $contador++;
        }
    }
...
$letra = 'S';
echo "\nNúmero de ciudades que comienzan con '$letra': " .
 contarCiudadesPorInicial($ciudades, $letra) . "\n";

 //Me retorna 2 por que cuenta tambien Sidney que esta en el arreglo
