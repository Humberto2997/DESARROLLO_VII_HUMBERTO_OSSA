...
function imprimirBiblioteca($libros) {
    foreach ($libros as $libro) {
        echo "</br>{$libro['titulo']} - {$libro['autor']} ({$libro['año']}) - {$libro['genero']} - " . 
...
echo "</br>Libros ordenados por año de publicación:</br>";
imprimirBiblioteca($biblioteca);
...
echo "</br>Libros ordenados alfabéticamente por título:</br>";
imprimirBiblioteca($biblioteca);
...
echo "</br>Libros de Ciencia ficción:</br>";
...
function generarReporteBiblioteca($biblioteca) {
  $reporte = [
    ...
    // Calcular libros por género
    foreach ($biblioteca as $libro) {
        $genero = $libro['genero'];
        if (!isset($reporte['libros_por_genero'][$genero])) {
            $reporte['libros_por_genero'][$genero] = 0;
        }
        $reporte['libros_por_genero'][$genero]++;
    }

    // Encontrar el autor con más libros
    $conteoAutores = array_count_values(array_column($biblioteca, 'autor'));
    arsort($conteoAutores); // Ordenar para que el primero sea el autor con más libros
