...
function calcularCrecimiento($ventas) {
  $ventasIniciales = $ventas[0]; 
  $ventasFinales = end($ventas); 
  if ($ventasIniciales == 0) return 0; 
  $crecimiento = (($ventasFinales - $ventasIniciales) / $ventasIniciales) * 100;
  return $crecimiento;
}

echo "\nCrecimiento de ventas por región y producto:</br>";
...
foreach ($ventas as $region => $productos) {
    ...
    foreach ($productos as $producto => $ventasProducto) {
        ...
        // Verificar si este producto tiene el mayor crecimiento
...
echo "Producto con mayor crecimiento: $productoConMayorCrecimiento</br>";
echo "Región con mayor crecimiento: $regionConMayorCrecimiento</br>";
echo "Porcentaje de crecimiento: " . number_format($mayorCrecimiento, 2) . "%\n";
